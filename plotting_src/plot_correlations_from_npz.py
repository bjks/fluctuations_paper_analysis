...
from plotting_src.correlation_from_joint import *
...
def get_corr_at_mdt(dts, rs, mdt):
    return rs[np.argmin(np.abs(dts - mdt))]
...
def plot_xy_correlation_list(correlation_function_list, x, y, 
    ...
    if corr_type=="cov":
        ...
        get_func=Correlation_function.get_corr_mle
        get_err_func=Correlation_function.get_corr_mle_err
    elif corr_type=="naive":
        get_func=Correlation_function.get_corr_naive
        get_err_func=None
    ...
    # =========== figure =========== #
    if fit:
        fig, ax0 = plt.subplots(1, 1, figsize=(8.3, 4.8))
...
#         ax.spines['left'].set_position('zero')
    ...
    for i, cf in enumerate(correlation_function_list):
        ...
        if color_by=="condition":
            condition = get_condition(labels[i])
            color = color_by_condition[condition]
        elif color_by=="promoter":
            promoter = get_promoter(labels[i])
        ...
        if scale_t:
            ...
            if mean_lambda!=None:
                mdt = np.log(2)/mean_lambda
                corr_at_mdt = get_corr_at_mdt(dts, rs, mdt)
                ax0.vlines(mdt, 0, corr_at_mdt, color=color, alpha=0.8)
                # ax0.hlines(corr_at_mdt, mdt, xlim[-1], color=color, alpha=0.8)
                ax0.scatter( mdt, corr_at_mdt, color=color, alpha=0.8)
        ...
        if err_style == "bar":
            ...
            ax0.plot(dts, rs, lw=2, color=color, label=label, alpha=0.5)
            ax0.fill_between(dts, rs-errs, rs+errs, color=color, alpha=0.2)
        elif err_style == None:
...
#         ax0.legend(ncol=2, bbox_to_anchor=(1,0.), loc="lower right")
    if legend:
        if color_by=="promoter":
            ncol=3
        else: 
            ncol=2
        ax0.legend(*legend_without_duplicate_labels(ax0), 
                   loc='lower center', bbox_to_anchor=(0.5, 1.), ncol=ncol, title=title)
...
def plot_corr_at_mdt(correlation_function_list, x, y, 
                                plot_file=None, 
                                label=[], 
...
#         ax.spines['left'].set_position('zero')
    ...
    mean_lambdas = list(mean_lambdas) + [None] * len(correlation_function_list)
    normalize = list(normalize) + [None] * len(correlation_function_list)
    if labels_for_legend==None:
        labels_for_legend = labels
    # =========== plot =========== #
    mean_lambdas_dic = {c:[] for c in conditions}
    corr_at_mdts_dic = {c:[] for c in conditions}
    colors = {c:[] for c in conditions}
    for i, cf in enumerate(correlation_function_list):
        condition = get_condition(labels[i])
        promoter = get_promoter(labels[i])
        ...
        # ax0.vlines(mdt, 0, corr_at_mdt, color=color, alpha=0.8)
        # ax0.hlines(corr_at_mdt, mdt, xlim[-1], color=color, alpha=0.8)
        # ax0.scatter( mean_lambda, corr_at_mdt, color=color, alpha=0.8, label=condition)
        mean_lambdas_dic[condition].append(mean_lambda)
        corr_at_mdts_dic[condition].append(corr_at_mdt)
        colors[condition].append(color)

        ax0.scatter(mean_lambda*60,corr_at_mdt, 
                    color=color_by_both[promoter][condition],label=promoter,
                    ec="black")
        
        
    
    # for condition in conditions:
    #     ax0.scatter(np.array(mean_lambdas_dic[condition])*60, corr_at_mdts_dic[condition], marker="o", 
    #                 color=colors[condition],label=condition, alpha=1) 
        # box1 = ax.boxplot(corr_at_mdts_dic[condition], positions=[np.median(np.array(mean_lambdas_dic[condition])*60)], manage_ticks=False, showfliers=False, patch_artist=True)
        # box2 = ax.boxplot(corr_at_mdts_dic[condition], positions=[np.median(np.array(mean_lambdas_dic[condition])*60)], manage_ticks=False, showfliers=False)
    
        # for item in ['boxes', 'whiskers', 'fliers', 'medians', 'caps']:
        #     plt.setp(box1[item], color=color_by_condition[condition], alpha=0.3)
        #     plt.setp(box2[item], color=color_by_condition[condition], alpha=0.7)


    ax0.set_xlabel(r"mean growth rate $\lambda$ (1/h)")
    ax0.set_ylabel(r"Correlation(dt=mdt)")
    ...
    for k in indict.keys():
        if filter==None or filter in k.split('_'):
            if keep=="both":
                ...
                out.append(k.split('_')[0].split("_")[0])
            elif keep=="promoter":
